...
if __name__ == "__main__":
    # Simple test for FilePriorityQueue
    import random
    import pprint
    import pickle

    # Create a queue in a temp directory
    queue = FilePriorityQueue(directory="test_queue_dir", max_memory_items=100, flush_threshold=101)

    # Push some items with random priorities
    print("Pushing items...")
    for i in range(10000):
        priority = random.randint(1, 1000)
        print(f"Pushing value={i}, priority={priority}")
        queue.push(priority, f"value_{i}")

    # Print the index file in readable format
    index_path = queue.index_path
    with open(index_path, "rb") as f:
        index_data = pickle.load(f)
    with open("index_output.txt", "w") as out_f:
        pprint.pprint(index_data, stream=out_f)
